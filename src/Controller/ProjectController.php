...
class ProjectController {
    ...
    public function createProject() {
        ...
        if ($name === '') Response(400, false, "Project name is required");
    ...
    public function deleteProject() {
        ...
        $data = json_decode(file_get_contents('php://input'), true);
        if (!isset($data['project_id'])) Response(400, false, "project_id is required");
        ...
        // ── Log ──────────────────────────────────────────────────────
        ActivityLogger::log('deleted_project', 'project',
            (int) $organization_id,
            (int) $data['project_id'], $project['name']);
    ...
    public function editProject() {
        ...
        if ($name === '') Response(400, false, "Project name cannot be empty");
        ...
        $result = $this->project->updateProject((int) $data['project_id'], $name, $description, $status);

        // ── Log ──────────────────────────────────────────────────────
        ActivityLogger::log('updated_project', 'project',
            (int) $organization_id,
            (int) $data['project_id'], $name);
    ...
    public function getProjectByID() {
        ...
        if (!$result) Response(404, false, "Project not found");
