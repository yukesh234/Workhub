...
class ActivityLogModel {
    ...
    private function createTable(): void {
        // PostgreSQL: SERIAL, VARCHAR CHECK instead of ENUM, separate CREATE INDEX
        $this->db->exec("
            CREATE TABLE IF NOT EXISTS activity_log (
                log_id       SERIAL PRIMARY KEY,
                actor_id     INT NOT NULL,
                actor_type   VARCHAR(10) NOT NULL CHECK (actor_type IN ('admin','user')),
        ...
        // Separate index creation (PostgreSQL does not support inline INDEX in CREATE TABLE)
        $this->db->exec("CREATE INDEX IF NOT EXISTS idx_activity_org    ON activity_log (org_id)");
        $this->db->exec("CREATE INDEX IF NOT EXISTS idx_activity_actor  ON activity_log (actor_id, actor_type)");
        $this->db->exec("CREATE INDEX IF NOT EXISTS idx_activity_ts     ON activity_log (created_at)");
    }
    ...
    public function getActionTypes(int $org_id): array {
        $stmt = $this->db->prepare("
            SELECT DISTINCT action FROM activity_log WHERE org_id = ? ORDER BY action
        ");
        $stmt->execute([$org_id]);
