...
class AnalyticsModel {
    ...
    $sql = <<<'SQL'
        SELECT
            (SELECT COUNT(*) FROM "user"  WHERE organization_id = ?) AS total_members,
            (SELECT COUNT(*) FROM project WHERE organization_id = ?) AS total_projects,
            (SELECT COUNT(*) FROM project WHERE organization_id = ? AND status = 'active')    AS active_projects,
            (SELECT COUNT(*) FROM project WHERE organization_id = ? AND status = 'completed') AS completed_projects,
            (SELECT COUNT(t.task_id)
                FROM task t JOIN project p ON p.project_id = t.project_id
                WHERE p.organization_id = ?)                          AS total_tasks,
            (SELECT COUNT(t.task_id)
                FROM task t JOIN project p ON p.project_id = t.project_id
                WHERE p.organization_id = ? AND t.status = 'completed') AS completed_tasks,
            (SELECT COUNT(t.task_id)
                FROM task t JOIN project p ON p.project_id = t.project_id
...
public function getProjectProgress(int $org_id): array {
   ...
   public function getMemberTasks(int $user_id, int $org_id): array {
    $sql = <<<'SQL'
        ...
        FROM task t
        JOIN project p ON p.project_id = t.project_id
        WHERE t.assigned_to = ? AND p.organization_id = ?
        ORDER BY
            CASE t.status
...
public function getMemberProjects(int $user_id): array {
    $sql = <<<'SQL'
        SELECT
            p.project_id, p.name, p.status, pm.role,
            COUNT(t.task_id)                                         AS total,
            SUM(CASE WHEN t.status = 'completed' THEN 1 ELSE 0 END) AS done
        FROM project_members pm
        JOIN project p ON p.project_id = pm.project_id
        LEFT JOIN task t ON t.project_id = p.project_id AND t.assigned_to = ?
        WHERE pm.user_id = ?
